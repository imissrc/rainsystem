# __all__ 的使用见https://blog.csdn.net/orangleliu/article/details/49848413
# https://segmentfault.com/q/1010000004204037
import os
import copy
...
class MMOCR:
    ...
    def readtext_for_video(self,
        ...
        if self.detect_model and self.recog_model:
            det_recog_result = self.det_recog_kie_inference(
                self.detect_model, self.recog_model, kie_model=self.kie_model)
            # 这个函数返回图像
            pp_result, res_img = self.det_recog_pp_for_video(det_recog_result)   # 类型都是 list()
        else:
            for model in list(
                    filter(None, [self.recog_model, self.detect_model])):
                result = self.single_inference(model, args.arrays,
                                               args.batch_mode,
                                               args.single_batch_size)
                pp_result, res_img = self.single_pp_for_video(result, model)

        return pp_result, res_img
    ...
    def det_recog_pp_for_video(self, result):
        final_results = []
        res_imgs = []
        args = self.args
        for arr, output, export, det_recog_result in zip(
    ...
    def single_pp_for_video(self, result, model):
        res_imgs = list()
        for arr, output, export, res in zip(self.args.arrays, self.args.output,
                                            self.args.export, result):
            if export:
            ...
            # 只写图片
            res_img = model.show_result(arr, res)  # 单张图片
            res_imgs.append(res_img)
            if self.args.imshow:
                mmcv.imshow(res_img, 'inference results')
            if self.args.print_result:
                print(res, end='\n\n')
        return result, res_imgs
...
# config_root="/home/lzh/cvdemo/models/detection"
# config_root=os.path.join(str(Path.cwd()), 'configs/')
config_root = os.path.join('./backend/textspot/mmocr', 'configs/')
ckpt_root = "/home/lzh/cvdemo/models/"

if MMOCR_MODEL is None:
    det_model_type = "DB_r50"
    det_config_dir = config_root+'textdet/'+'dbnet/dbnet_r50dcnv2_fpnc_1200e_icdar2015.py'
    det_ckpt_dir = ckpt_root+'detection/dbnet_r50dcnv2_fpnc_sbn_1200e_icdar2015_20210325-91cef9af.pth'

    recog_model_type = "CRNN"
    recog_config_dir = config_root+'textrecog/'+'crnn/crnn_academic_dataset.py'
    recog_ckpt_dir = ckpt_root+'CRNN/crnn_academic-a723a1c5.pth'


    # 初始化模型
    MMOCR_MODEL = MMOCR(det=det_model_type,
                        det_config=det_config_dir,
                        det_ckpt=det_ckpt_dir,
                        recog=recog_model_type,
                        recog_config=recog_config_dir,
                        recog_ckpt=recog_ckpt_dir)

def build_mmocr_model():
    return MMOCR_MODEL